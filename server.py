...
def process_referral_rewards(db: Session, new_user: User, deposit_amount: float, deposit_type: str):
    ...
    # Exit if the new user has no referral code
    ...
    reward_amount = deposit_amount * (settings.referrer_reward_percent / 100)

    # Create a referral reward entry for the referrer
    referrer_reward = ReferralReward(
        referrer_id=referrer.id,
        referred_id=new_user.id,
        reward_type=deposit_type,  # use deposit type as reward type
        reward_amount=reward_amount,
        status="paid"
    )
    db.add(referrer_reward)

    # Add reward to the referrer's crypto balance
    if deposit_type == "bitcoin":
        ...
        raise ValueError(f"Unsupported crypto type: {deposit_type}")
...
async def reset_password(
    ...
    """Reset password using OTP with alert email"""
    ip_address = request.client.host if hasattr(request, "client") else "unknown"
    user_agent = request.headers.get("user-agent", "Unknown")
    location = get_location_from_ip(ip_address)
    ...
    try:
        ...
        # Log security event
        log_security_event(db, user.id, "password_reset", "Password reset using OTP", ip_address)
...
async def reset_pin(
    ...
    """Reset PIN using OTP with alert email"""
    ip_address = request.client.host if hasattr(request, "client") else "unknown"
    user_agent = request.headers.get("user-agent", "Unknown")
    location = get_location_from_ip(ip_address)
    ...
    try:
        ...
        # Log security event
        log_security_event(db, user.id, "pin_reset", "PIN reset using OTP", ip_address)
...
async def confirm_deposit(
    ...
    if action == "confirm":
        ...
        # ✅ Trigger referral rewards (only if this user was referred)
    ...
    # Send email to user with deposit details and status
    try:
        template_vars = {
            "deposit_id": deposit.id,
            "crypto_type": deposit.crypto_type,
...
def update_admin_settings(
    ...
    # Fields allowed to update (percentage-based)
    for field in [
        "bitcoin_rate_usd",
        "ethereum_rate_usd",
        "global_mining_rate",
        ...
        "referral_reward_type",        # bitcoin or ethereum
        "referrer_reward_percent",     # reward % for referrer only
    ]:
        if field in data:
...
async def confirm_withdrawal(
    ...
    """Confirm or reject a withdrawal and notify the user with full details"""
    ...
    # Get admin settings for rates
    admin_settings = db.query(AdminSettings).first()
    if not admin_settings:
        raise HTTPException(status_code=500, detail="Admin settings not configured")
    
    if withdrawal.crypto_type.lower() == "bitcoin":
        usd_amount = float(withdrawal.amount * Decimal(admin_settings.bitcoin_rate_usd))
    else:
        usd_amount = float(withdrawal.amount * Decimal(admin_settings.ethereum_rate_usd))
    ...
    if action == "confirm":
        ...
        try:
            email_service.send_template_email(
                to_email=user.email,
                subject=f"Withdrawal Approved - {withdrawal.crypto_type}",
            ...
            logger.error(f"Failed to send withdrawal approval email to user: {e}")
        ...
        try:
            email_service.send_template_email(
                to_email=user.email,
                subject=f"Withdrawal Rejected - {withdrawal.crypto_type}",
                template_name="withdrawal_status_alert.html",
                template_vars={
                    "withdrawal_id": withdrawal.id,
                    "crypto_type": withdrawal.crypto_type,
            ...
            logger.error(f"Failed to send withdrawal rejection email to user: {e}")
