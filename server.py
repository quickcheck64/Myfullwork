...
class AdminSettings(Base):
    ...
    referrer_reward_percent = Column(Float, default=10.0)  # ✅ Default changed to 10%
    referral_reward_amount = Column(Float, default=0.0)  # Flat reward amount
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://smarts9trading.netlify.app"
    ],
...
@app.on_event("startup")
@repeat_every(seconds=30)  # runs every 30 seconds
def mining_background_task() -> None:
    """
    Background task to process mining for all users automatically.
    ...
    try:
        ...
        # Fetch all active sessions joined with users in one query
        active_sessions = (
            db.query(MiningSession, User)
            .join(User, MiningSession.user_id == User.id)
            .filter(MiningSession.is_active == True)
            .all()
        )
        ...
        for session, user in active_sessions:
            try:
                # Skip paused users
                if getattr(user, "mining_paused", False):
                    continue

                deposited_amount = Decimal(session.deposited_amount)
                mining_rate_fraction = Decimal(session.mining_rate) / Decimal(100)
                total_per_day = deposited_amount * mining_rate_fraction
                seconds_in_day = Decimal(24 * 3600)
                ...
                crypto_type = session.crypto_type.lower().strip()
                if crypto_type in ["bitcoin", "btc"]:
                    user.bitcoin_balance = (user.bitcoin_balance or Decimal(0)) + mined_amount
                    balance = float(user.bitcoin_balance)
                elif crypto_type in ["ethereum", "eth"]:
                    user.ethereum_balance = (user.ethereum_balance or Decimal(0)) + mined_amount
                    balance = float(user.ethereum_balance)
                else:
                    print(f"⚠️ Unsupported crypto type: {session.crypto_type}")
                    continue
                ...
                print(f"⚠️ Failed processing session {session.id} for user {user.id}: {e}")
                continue

        # Commit all updates once
        db.commit()
        print(f"✅ Background mining synced. Total mined: {float(total_mined)}")

    except Exception as e:
        db.rollback()
        print(f"❌ Error in background mining task: {e}")
