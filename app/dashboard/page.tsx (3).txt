...
export default function CryptoMiningDashboard() {
  ...
  const formatActivityMessage = (activity: ActivityItem) => {
    const cryptoSymbol = getCryptoSymbol(activity.type)
    const amount = formatCrypto(activity.message, 6)
    const cryptoName = activity.type ? activity.type.charAt(0).toUpperCase() + activity.type.slice(1) : ""
  ...
  const calculateReferralRewardsByCrypto = () => {
    ...
    const rewards = allTransactions
      ...
      .reduce(
        (acc, t) => {
          const cryptoType = t.type?.toLowerCase()
          if (cryptoType === "bitcoin") {
            acc.bitcoin += Number(t.message || 0)
          } else if (cryptoType === "ethereum") {
            acc.ethereum += Number(t.message || 0)
          }
          return acc
        },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatCurrency(dashboardAnalytics.portfolio_overview.bitcoin_balance_usd)}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatCurrency(dashboardAnalytics.portfolio_overview.ethereum_balance_usd)}
                  </p>
                </CardContent>
              </Card>
            </div>
