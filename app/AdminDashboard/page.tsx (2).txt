...
export default function AdminDashboard() {
  ...
  const uploadQRMutation = useMutation({
    mutationFn: (formData: FormData) => {
      return fetch("/api/admin/upload-qr-code", {
        method: "POST",
        headers: {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-settings"] })
      setQrUploadForm({ crypto_type: "bitcoin", wallet_address: "", qr_file: null })
    },
    onError: (error: any) => {
      toast({
        title: "Error",
  ...
  const handleQRUpload = () => {
    if (!qrUploadForm.qr_file || !qrUploadForm.wallet_address) {
      toast({
        title: "Error",
        description: "Please select a file and enter wallet address",
        variant: "destructive",
      })
      return
    }
    ...
    formData.append("crypto_type", qrUploadForm.crypto_type)
    formData.append("wallet_address", qrUploadForm.wallet_address)
    formData.append("qr_file", qrUploadForm.qr_file)
  ...
  const {
    ...
    queryFn: async () => {
      console.log("[v0] Fetching pending withdrawals...")
      ...
      console.log("[v0] Withdrawal data:", data)
      return data
    },
    // enabled: activeTab === "withdrawals", // Only fetch when on withdrawals tab
  })
  ...
  const withdrawalActionMutation = useMutation({
    mutationFn: async ({ withdrawalId, action }: { withdrawalId: number; action: "confirm" | "reject" }) => {
      const response = await fetch(`/api/admin/withdrawals/${withdrawalId}/confirm?action=${action}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
    ...
    onSuccess: (data, { action }) => {
      ...
      refetchWithdrawals() // Use refetch instead of manual fetch
    },
    onError: (error) => {
      toast({
        title: "Error",
                ...
                <CardContent className="p-6 space-y-6">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        value={qrUploadForm.wallet_address}
                        onChange={(e) => setQrUploadForm((prev) => ({ ...prev, wallet_address: e.target.value }))}
                        placeholder="Enter wallet address for this QR code"
                        className="font-mono text-sm"
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setQrUploadForm((prev) => ({ ...prev, qr_file: file }))
                        }}
                        className="w-full p-2 border rounded-md"
                      />
                      <p className="text-xs text-muted-foreground">Supported formats: JPEG, PNG, GIF</p>
                    </div>

                    <Button
                      onClick={handleQRUpload}
                      disabled={uploadQRMutation.isPending || !qrUploadForm.qr_file || !qrUploadForm.wallet_address}
                      className="w-full"
                    >
                      <Save className="h-4 w-4 mr-2" />
                  ...
                  <div className="mt-6 pt-6 border-t">
                    ...
                    {qrCodesLoading ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {qrCodes.map((qr) => (
                          <div key={qr.id} className="border rounded-lg p-4 space-y-2">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="font-mono text-xs break-all">{qr.wallet_address}</p>
                            </div>
                            <div className="space-y-1">
                              <p className="text-xs text-muted-foreground">Created:</p>
                              <p className="text-xs">{new Date(qr.created_at).toLocaleDateString()}</p>
                            </div>
                            {qr.qr_code_url && (
                              <div className="mt-2">
                                <img
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label className="text-sm font-medium">
                          Referral Reward {settings.referral_reward_type === "percentage" ? "(%)" : "($)"}
                        </Label>
                        <Input
                          type="number"
                          step="0.01"
                          ...
                          onChange={(e) =>
                            setSettings((prev) => ({
                              ...prev,
                              referral_reward_amount: Number.parseFloat(e.target.value) || 0,
                            }))
                          }
                          placeholder="Amount for new referrals"
                        />
                      </div>

                      <div className="space-y-2">
                        <Label className="text-sm font-medium">
                          Referrer Reward {settings.referral_reward_type === "percentage" ? "(%)" : "($)"}
                        </Label>
                        <Input
                          type="number"
                          step="0.01"
                          ...
                          onChange={(e) =>
                            setSettings((prev) => ({
                              ...prev,
                              referrer_reward_amount: Number.parseFloat(e.target.value) || 0,
                            }))
                          }
                          placeholder="Amount for referrers"
                        />
                      </div>
                    </div>
                  </>
            ...
            <Card className="shadow-lg">
              ...
              <CardContent className="p-6">
                <div className="mb-4 p-2 bg-gray-100 rounded text-sm">
                  ...
                  <p>Debug: Withdrawals loading: {withdrawalsLoading.toString()}</p>
                  <p>Debug: Pending withdrawals count: {pendingWithdrawals?.length || 0}</p>
                </div>
